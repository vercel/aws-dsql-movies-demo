...
async function main() {
  ...
  try {
    ...
    for (const file of migrationFiles) {
      const result = await client.query('SELECT name FROM migrations WHERE name = $1', [file]);
      ...
      if (result.rows.length === 0) {
        ...
        const maxId = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM migrations');
        await client.query(
